...
// vec4 slots needed to hold MAX_TYPES scalars (packed 4-per-vec4 in uniforms).
const TYPE_VEC4 = Math.ceil(MAX_TYPES / 4);
// vec4 slots for the MAX_TYPES×MAX_TYPES affinity table.
const AFF_VEC4  = Math.ceil((MAX_TYPES * MAX_TYPES) / 4);
// Diagnostic readback: pos(2)+vel(2)+typeId+speed=6, then typeForce[N], transformProb[N].
const DIAG_FLOATS = 6 + 2 * MAX_TYPES;
const DIAG_BYTES  = Math.ceil(DIAG_FLOATS / 4) * 16;
...
// MAX_TYPES visually distinct colours, used in both JS (UI) and WGSL (render).
// The first 20 are a hand-tuned palette (so low type counts look good); the rest
// are generated by golden-angle hue spread. The WGSL COLORS array is generated
// from this same list, so they stay in sync.
export const TYPE_COLORS_HEX: string[] = (() => {
    ...
    const out = base.slice(0, MAX_TYPES);
    for (let i = base.length; i < MAX_TYPES; i++) {
        const h = (i * 137.508) % 360;          // golden-angle hue
        const s = 65 + (i % 3) * 12;            // 65–89%
        const l = 45 + ((i % 5) - 2) * 7;       // ~31–59%
        out.push(hslToHex(h, s, l));
    }
    return out;
})();

// WGSL array literal "vec4f(r,g,b,1.0), ..." for COLORS, derived from the hex list.
function colorsWGSL(): string {
    return TYPE_COLORS_HEX.map(hex => {
        const r = parseInt(hex.slice(1, 3), 16) / 255;
        const g = parseInt(hex.slice(3, 5), 16) / 255;
        const b = parseInt(hex.slice(5, 7), 16) / 255;
        return `vec4f(${r.toFixed(3)},${g.toFixed(3)},${b.toFixed(3)},1.0)`;
    }).join(', ');
}
...
function randThreshMag(): number {
    ...
    if (Math.random() < 0.22) m = 2 + Math.pow(Math.random(), 3) * 48; // ~22%: tail 2..50, strongly low-biased
...
export class ParticleSimulation {
    ...
    private generateTypeMassData(): Uint32Array<ArrayBuffer> {
        const data = new Uint32Array(TYPE_VEC4 * 4);  // padded to the uniform's vec4u size
    ...
    private generatePatchConfigData(): ArrayBuffer {
        const ab  = new ArrayBuffer(32 + TYPE_VEC4 * 16);  // 8 leading f32 + patchCount vec4u[]
    ...
    // Mode 3 per-type/affinity tables uniform. Layout matches the WGSL struct:
    //   affinity[from*MAX_TYPES + to]   (array<vec4f, AFF_VEC4>)
    //   per-type bond strength          (array<vec4f, TYPE_VEC4>)
    //   per-type bond rest dist         (array<vec4f, TYPE_VEC4>)
    private generatePatchTablesData(): ArrayBuffer {
        const ab  = new ArrayBuffer((AFF_VEC4 + 2 * TYPE_VEC4) * 16);
        const f32 = new Float32Array(ab);
        const strBase  = AFF_VEC4 * 4;            // bondStr starts on the next vec4 boundary
        const distBase = strBase + TYPE_VEC4 * 4; // bondDist after bondStr
        for (let i = 0; i < MAX_TYPES * MAX_TYPES; i++) f32[i] = this.patchAffinity[i] ?? 0;
        for (let t = 0; t < MAX_TYPES; t++) {
            f32[strBase  + t] = this.patchTypeBondStr[t]  ?? 0;
            f32[distBase + t] = this.patchTypeBondDist[t] ?? 26;
        }
        return ab;
    }
    ...
    private async createBuffers(): Promise<void> {
        ...
        this.diagOutputBuffer  = this.device!.createBuffer({ label: 'diagOutput',  size: DIAG_BYTES, usage: GPUBufferUsage.STORAGE | GPUBufferUsage.COPY_SRC });
        this.diagStagingBuffer = this.device!.createBuffer({ label: 'diagStaging', size: DIAG_BYTES, usage: GPUBufferUsage.MAP_READ | GPUBufferUsage.COPY_DST });
        ...
        // Mode 2 (mass) buffers
        this.typeMassBuffer      = this.device!.createBuffer({ label: 'typeMass',       size: TYPE_VEC4 * 16,  usage: GPUBufferUsage.UNIFORM  | GPUBufferUsage.COPY_DST });
        ...
        this.patchConfigBuffer = this.device!.createBuffer({ label: 'patchConfig', size: 32 + TYPE_VEC4 * 16, usage: GPUBufferUsage.UNIFORM | GPUBufferUsage.COPY_DST });
        this.queue!.writeBuffer(this.patchConfigBuffer, 0, this.generatePatchConfigData());
        this.patchTablesBuffer = this.device!.createBuffer({ label: 'patchTables', size: (AFF_VEC4 + 2 * TYPE_VEC4) * 16, usage: GPUBufferUsage.UNIFORM | GPUBufferUsage.COPY_DST });
    ...
    private getComputeShaderCode(): string {
        return /* wgsl */`
            ...
            fn main(@builtin(global_invocation_id) id: vec3u) {
                ...
                var accel  = vec2f(0.0);
                var typeForce: array<f32, ${MAX_TYPES}>;
                ...
                for (var goy = -1; goy <= 1; goy++) {
                    for (var gox = -1; gox <= 1; gox++) {
                        ...
                        for (var k = start; k < end; k++) {
                            ...
                            let f     = forces[myType * ${MAX_TYPES}u +otherType];
                ...
                if (simMode == 1u || simMode == 2u) {
                    ...
                    for (var t: u32 = 0u; t < numTypes; t++) {
                        let rule = transformRules[myType * ${MAX_TYPES}u +t];
    ...
    private getDiagnosticShaderCode(): string {
        return /* wgsl */`
            ...
            struct DiagOutput {
                ...
                typeId:        f32,
                speed:         f32,
                typeForce:     array<f32, ${MAX_TYPES}>,
                transformProb: array<f32, ${MAX_TYPES}>,
            }
            ...
            fn main() {
                ...
                for (var t: u32 = 0u; t < ${MAX_TYPES}u; t++) {
                    output.typeForce[t] = typeForce[t];
                    let rule = tRules[myType * ${MAX_TYPES}u +t];
                    var prob: f32 = 0.0;
    ...
    private getMode2TransformShaderCode(): string {
        return /* wgsl */`
            ...
            fn main(@builtin(global_invocation_id) id: vec3u) {
                ...
                if (dstType >= ${MAX_TYPES}u || srcType == dstType) {
    ...
    private getMode3ShaderCode(): string {
        return /* wgsl */`
            ...
            struct PatchTables {
                affinity: array<vec4f, ${AFF_VEC4}>,  // [from*20 + to]
                bondStr:  array<vec4f, ${TYPE_VEC4}>,    // per-type bond strength
                bondDist: array<vec4f, ${TYPE_VEC4}>,    // per-type bond rest length
            }
            ...
            fn affinityOf(a: u32, b: u32) -> f32 { let i = a * ${MAX_TYPES}u +b; return tab.affinity[i >> 2u][i & 3u]; }
            ...
            fn main(@builtin(global_invocation_id) id: vec3u) {
                ...
                var accel  = vec2f(0.0);
                var torque = 0.0;
                var typeForce: array<f32, ${MAX_TYPES}>;  // per-type accumulated force (Mode 4 & 5 transforms)
    ...
    update(): void {
        ...
        if (doDiag) {
            enc.copyBufferToBuffer(this.diagOutputBuffer!, 0, this.diagStagingBuffer!, 0, DIAG_BYTES);
        }
        ...
        if (doDiag) {
            ...
            this.diagStagingBuffer!.mapAsync(GPUMapMode.READ).then(() => {
                ...
                const data: DiagnosticData = {
                    ...
                    transformProbs: Array.from({ length: n }, (_, t) => f32[6 + MAX_TYPES + t]),
                };
